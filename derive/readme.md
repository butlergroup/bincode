# bincode_reloaded-derive

The derive crate for bincode_reloaded. Implements `bincode_reloaded::Encode` and `bincode_reloaded::Decode`.
...
For testing purposes, all generated code is outputted to the current `target/generated/bincode_reloaded` folder, under file name `<struct/enum name>_Encode.rs` and `<struct/enum name>_Decode.rs`. This can help with debugging.
