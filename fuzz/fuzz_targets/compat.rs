...
fuzz_target!(|data: &[u8]| {
    let config = bincode_reloaded::config::legacy().with_limit::<1024>();
    #[allow(deprecated)]
    let mut configv1 = bincodev1::config();
    configv1.limit(1024);
    let bincode_v1: Result<AllTypes, _> = configv1.deserialize_from(data);
    let bincode_v2: Result<(AllTypes, _), _> = bincode_reloaded::decode_from_slice(data, config);

    match (&bincode_v1, &bincode_v2) {
        (Err(e), _) if e.to_string() == "the size limit has been reached" => {}
        (_, Err(bincode_reloaded::error::DecodeError::LimitExceeded)) => {}
        (Ok(bincode_v1), Ok((bincode_v2, _))) if bincode_v1 != bincode_v2 => {
            println!("Bytes:      {:?}", data);
            println!("bincode_reloaded V1: {:?}", bincode_v1);
            println!("bincode_reloaded V2: {:?}", bincode_v2);
